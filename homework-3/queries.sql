...
SELECT customers.company_name AS custtomers, CONCAT(employees.first_name, ' ', employees.last_name) AS employee
FROM customers
...
FROM (SELECT product_name, quantity FROM products JOIN order_details USING(product_id)) AS orders
